...
In general gpx looks like this
...
### <a name='geoUri'></>geoUri=geoUri : (xml only) contains info in geo-uri format
...
### <a name='infer'></>infer=1 : (xml with geoUri only) try to infer lat,lon and name from description
